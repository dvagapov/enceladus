  ...
  "a local mapping table" should {
    ...
    "be created" when {
      ...
      "a struct type target attribute is provided" in {
        val localMt = LocalMappingTable(dfComplexMt, Seq("id"), "sval", Nil)
      ...
      "a join key is inside a struct" in {
        val localMt = LocalMappingTable(dfComplexMt, Seq("sval.e"), "sval", Nil)
      ...
      "a join condition having 2 keys" in {
        val localMt = LocalMappingTable(dfComplexMt, Seq("id", "sval.e"), "sval", Nil)
    ...
    "throw an exception" when {
      ...
      "a join key does not exists in the schema" in {
        intercept[IllegalArgumentException] {
          LocalMappingTable(dfMt, Seq("dummy"), "val", Nil)
        }
      }

      "a target attribute provided does not exist in the schema" in {
        intercept[IllegalArgumentException] {
          LocalMappingTable(dfMt, Seq("id"), "dummy", Nil)
        }
      }

      "a join key is an array" in {
        intercept[IllegalArgumentException] {
          LocalMappingTable(dfMt, Seq("arr"), "val", Nil)
        }
      }

      "a join key is a struct" in {
        intercept[IllegalArgumentException] {
          LocalMappingTable(dfComplexMt, Seq("sval"), "val", Nil)
        }
      }

      "a join key is inside an array" in {
        intercept[IllegalArgumentException] {
          LocalMappingTable(dfComplexMt, Seq("arst.a"), "val", Nil)
        }
      }

      "a target attribute is an array" in {
        intercept[IllegalArgumentException] {
          LocalMappingTable(dfMt, Seq("id"), "arr", Nil)
        }
      }

      "a target attribute is inside an array" in {
        intercept[IllegalArgumentException] {
          LocalMappingTable(dfComplexMt, Seq("id"), "arst.a", Nil)
        }
      }
